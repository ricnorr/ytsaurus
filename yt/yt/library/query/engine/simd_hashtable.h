...
struct simd_hashtable_t {
    ...
    while (table[(offset + hash) % table.size()] != INT_MIN) {
      ...
      if (((offset + hash) % table.size()) == start_index) {
        throw std::runtime_error(
            "scalar insert cycle,offset=" + std::to_string(offset) +
  ...
  size_t simd_find(const std::vector<element_t> &S_keys,
                   std::vector<element_t> S_found) {
    return simd_find(S_keys.data(), S_keys.size(), S_found.data(),
                     S_found.size());
  }
  
  size_t simd_find(const element_t *S_keys, size_t S_keys_size,
                   element_t *S_found, size_t S_found_size) const {
    const int table_size = table.size();
    assert(table_size % 2 == 0);
    assert(S_found_size >= S_keys_size);
    size_t S_keys_ind = 0;
    ...
    while (S_keys_ind + 7 < S_keys_size) {
      // std::cout << "iter" << std::endl;
      // std::cout << "find " << S_keys_ind << std::endl;
      // load only where inv = 1
      auto new_key = _mm256_maskload_epi32(&S_keys[S_keys_ind], invalid);
      ...
      _mm256_maskstore_epi32(&S_found[S_found_ind], permuted_eq_mask,
                             permuted_key);
