...
public:
    ...
    //   auto values_cnt = inExpr->Values.size();
      simd_hashtable_t simd_hashtable(10);
      ...
      return [hashtable = std::move(simd_hashtable)](
        ...
        while (true) {
            ...
            std::cout << "ricnorr, handle batch size " << rows.size() << std::endl;
            size_t cur_batch_size = 0;
            for (TUnversionedRow row : rows) {
              std::cout << "ricnorr, handle row" << std::endl;
              assert(row.Elements().Size() == 1);
              auto &element = row.Elements()[0];
              assert(element.Type == EValueType::Int64);
              auto integralValue = element.Data.Int64;
              buffer[cur_batch_size] = integralValue;
              cur_batch_size++;
              if (cur_batch_size == buffer_size) {
                auto found_size = hashtable.simd_find(buffer, cur_batch_size, ok_buffer, buffer_size);
                for (size_t i = 0; i < found_size; i++) {
                  auto row = rowBuffer->AllocateUnversioned(1);
                  row[0] = rowBuffer->CaptureValue(
                      NTableClient::MakeUnversionedInt64Value(
                          static_cast<int32_t>(ok_buffer[i])));
                //    writer->Write()
                }
              }
              bool ok = hashtable.scalar_find(static_cast<int32_t>(integralValue));
              if (ok) {
                auto row = rowBuffer->AllocateUnversioned(1);
                row[0] = rowBuffer->CaptureValue(
                    NTableClient::MakeUnversionedInt64Value(static_cast<int32_t>(integralValue)));
                result.push_back(row);
              }
            }
        }

        std::cout << "ricnorr, write rows" << std::endl;
        auto& writer = ctx->Writer;
        std::ignore = writer->Write(result);
        std::ignore = writer->Close();
        std::cout << "ricnorr, end iter" << std::endl;
      };
    }
        ...
        TFuture<TFeatureFlags> responseFeatureFlags) override
    {
        ...
        try {
            ...
            if (isInSimdQuery(query)) {
              ...
              queryInstance.Run(fragmentParams.GetLiteralValues(),
                                fragmentParams.GetOpaqueData(),
                                fragmentParams.GetOpaqueDataSizes(),
                                &executionContext);
            }
